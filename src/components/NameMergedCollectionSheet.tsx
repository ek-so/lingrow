...
interface NameMergedCollectionSheetProps {
  open: boolean
  initialName: string
...
export function NameMergedCollectionSheet({
  open,
  initialName,
  onConfirm,
  onCancel,
}: NameMergedCollectionSheetProps) {
  const inputId = useId()
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center overflow-hidden sm:items-center">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="name-merged-title"
      ...
      >
        ...
        <h2 id="name-merged-title" className="text-lg font-semibold tracking-tight">
          Name merged set
        </h2>
